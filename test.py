...
def save_frame_camera_cycle(dir_path, delay=1, window_name='frame'):
    ...
    with open('dataset.csv','w') as f:
        writer = csv.writer(f)
        writer.writerow(["image_path","image_time"])
    f.close()
    ...
    while True:
        ...
        now_time = datetime.datetime.now()
        hour_time = now_time.minute
        if (hour_time >= 0 and hour_time < 0) or (hour_time >= 37 and hour_time < 60):
            timeFlag = 1
            if (hour_time == 38 and dayFlag == 0):
                print('into -------------------------------------') 
            ...
            next_time = datetime.datetime(now_time.year,now_time.month,now_time.day,now_time.hour,now_time.minute,0)
            ...
            if (timeFlag == 0):
                ...
                core_time = now_time - base_time
                core_time = core_time.seconds
                if (core_time >= 1):
                    base_time = datetime.datetime.now()
                    print("1")
                    os.makedirs(path,exist_ok=True)
...
if __name__ == "__main__":
    path = '/Users/yanokoutarou/workspace/ゼミ/code/test/'
    save_frame_camera_cycle(path)